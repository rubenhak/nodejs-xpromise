...
describe('Promise.execute', () => {
    ...
    it('case-big-all-pass', () => {
        ...
        return Promise.execute(items, (x) => {
            ...
            const expectedProcessedMap = _.makeDict(items, x => x.id, x => true);
            should(processed).be.eql(expectedProcessedMap);

            const resultAsMap = _.makeDict(result, x => x, x => true);
    ...
    it('case-big-some-fail-several-times', () => {
        ...
        return Promise.execute(items, (x) => {
            ...
            processed[x.id] = true;
            return 'X' + x.id;
        }, { retryCount: 3, coolDownOnFailure: true }).then((result) => {
            const expectedProcessedMap = _.makeDict(items, x => x.id, x => true);
            should(processed).be.eql(expectedProcessedMap);
